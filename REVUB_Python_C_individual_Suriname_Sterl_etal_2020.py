...
ax1.plot(rule_curve_head_series[:,plot_HPP], color = colour_CONV)
...
if STOR_break[plot_HPP] == 0:
    ...
    ax1.legend(['rule curve', 'BAL', 'STOR'])
else:
    ax1.legend(['rule curve', 'BAL'])
...
ax1.plot(rule_curve_volume_series[:,plot_HPP], color = colour_CONV)
...
# [plot] number of turbines in use

# [calculate] capacity of single turbine (MW)
P_unit = 30
no_turbines = 6
...
no_turbines_used_BAL = np.full([int(np.max(sum(days_year)))*hrs_day,len(simulation_years),HPP_number], np.nan)
no_turbines_used_STOR = np.full([int(np.max(sum(days_year)))*hrs_day,len(simulation_years),HPP_number], np.nan)

# [loop] over all turbines
for n in range(no_turbines):
    m = no_turbines - n
    no_turbines_used_BAL[P_BAL_hydro_total <= m*P_unit] = m
    
    if STOR_break[plot_HPP] == 0:
        no_turbines_used_STOR[P_STOR_hydro_total <= m*P_unit] = m

# [write] time series of number of turbines in use
no_turbines_used_BAL_series = no_turbines_used_BAL[:,:,plot_HPP]
no_turbines_used_BAL_series = (np.transpose(no_turbines_used_BAL_series)).ravel()
no_turbines_used_BAL_series = no_turbines_used_BAL_series[np.isfinite(no_turbines_used_BAL_series)]
# [create] histogram of number of turbines in use
no_turbines_used_pdf_BAL = np.histogram(no_turbines_used_BAL_series, bins = np.array(range(1,no_turbines + 2)))
no_turbines_used_pdf_BAL = (no_turbines_used_pdf_BAL[0])/np.sum(no_turbines_used_pdf_BAL[0])

if STOR_break[plot_HPP] == 0:
    # [write] time series of number of turbines in use
    no_turbines_used_STOR_series = no_turbines_used_STOR[:,:,plot_HPP]
    no_turbines_used_STOR_series = (np.transpose(no_turbines_used_STOR_series)).ravel()
    no_turbines_used_STOR_series = no_turbines_used_STOR_series[np.isfinite(no_turbines_used_STOR_series)]
    # [create] histogram of number of turbines in use
    no_turbines_used_pdf_STOR = np.histogram(no_turbines_used_STOR_series, bins = np.array(range(1,no_turbines + 2)))
    no_turbines_used_pdf_STOR = (no_turbines_used_pdf_STOR[0])/np.sum(no_turbines_used_pdf_STOR[0])

fig = plt.figure()
plt.bar(np.array(range(no_turbines)),no_turbines_used_pdf_BAL, label = 'BAL', width = 0.8)
if STOR_break[plot_HPP] == 0:
    plt.bar(np.array(range(no_turbines)),no_turbines_used_pdf_STOR, label = 'STOR', width = 0.6)
plt.legend(loc = 'center left', bbox_to_anchor = (1, 0.5))
plt.xticks(np.array(range(no_turbines)), np.array(range(no_turbines)) + 1)
